...
describe "Level5" do
  ...
  describe "features" do
    ...
    specify "all features of level 3" do
      Level5.new.is_a?(Level3).should be_truthy
    end
    ...
    specify "there is a seat view" do
      ...
      @level.loaded_passenger.should be_falsey
    end

    specify "there is a waiting platform" do
      @level.waiting.x.should == 50
      @level.waiting.y.should == 250
      @level.platforms.include?(@level.waiting).should be_truthy
    end
    ...
    specify "there is 2 passengers on the waiting platform" do
      ...
      object_on_platform?(first, @level.waiting).should be_truthy
      second = @level.passengers[1]
      object_on_platform?(second, @level.waiting).should be_truthy
    ...
    specify "passenger goes down the lem when the lem is on the target platform" do
      ...
      object_on_platform?(passenger, @level.target).should be_truthy
    end
    ...
    specify "reset" do
      ...
      @level.reset
      @level.loaded_passenger.should be_nil
      object_on_platform?(@level.passengers[0], @level.waiting).should be_truthy
      object_on_platform?(@level.passengers[1], @level.waiting).should be_truthy
    end
